...
_PluginManager.prototype.init = function (page) {
    ...
    for (var i in files) {
        if (files[i] && Utils.endsWith(files[i], "js")) {
            ...
            self.pluginsStatus[p.name] = true;
        }
    }

    for (var i in disabledPlugins) {
        self.pluginsStatus[disabledPlugins[i]] = false;
    }
};
...
_PluginManager.prototype.disable = function (plugins) {
    disabledPlugins = plugins;
};

_PluginManager.prototype.enable = function (plugins) {
    var self = this;
    for (var i in plugins) {
        self.pluginsStatus[plugins[i]] = true
    }
};
...
_PluginManager.prototype.getResults = function () {
    ...
    var results = {};
    for (var p in self.plugins) {
        console.log(p + " " + self.pluginsStatus[p])
        if (!self.pluginsStatus[p]) continue;
        try {
